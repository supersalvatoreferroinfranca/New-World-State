...
export default function ModerationPanelModal({
  ...
  const handleApprove = (art: NewsArticle) => {
    if (confirm(`Confermi l'approvazione e la pubblicazione dell'articolo "${art.title}"?`)) {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-serif text-lg font-bold text-brand-gold leading-tight">
                  Pannello Moderazione Giornalistica — Custodi Digitali
                </h2>
                <p className="text-[10px] text-slate-300 font-tech tracking-wider uppercase">
                  Revisione e approvazione articoli inoltrati dai Cronisti
                </p>
              </div>
            </div>
            ...
            {articles.length === 0 ? (
              <div className="text-center py-12 bg-slate-50 border border-dashed border-slate-300 rounded-2xl p-8">
                ...
                <h3 className="font-serif text-base font-bold text-[#0a1c3e]">
                  {tab === 'pending' ? 'Nessun Articolo in Attesa' : 'Nessun Articolo Trovato'}
                </h3>
                <p className="text-xs text-slate-500 max-w-md mx-auto mt-1">
                  {tab === 'pending'
                    ? 'Tutti gli articoli inviati dai cronisti sono stati moderati. Nuovi invii appariranno automaticamente qui.'
                    : 'Non ci sono articoli registrati nel sistema.'}
                </p>
              </div>
            ) : (
                ...
                {articles.map((art) => (
                  ...
                  >
                      ...
                      <div>
                        <div className="flex flex-wrap items-center gap-2 mb-1">
                          ...
                          <span className="text-[10px] font-mono text-slate-400">
                            Invio: {new Date(art.createdAt).toLocaleString('it-IT')}
                          </span>
                        </div>
                    ...
                    <div className="flex flex-wrap items-center justify-between pt-2 border-t border-slate-100 text-xs text-slate-500">
                      <div>
                        Redatto da: <span className="font-bold text-[#0a1c3e]">{art.authorName}</span> ({art.authorRole || 'Cronista'})
                      </div>

                      <div className="flex items-center gap-3">
                        <span>📷 {art.images?.length || 0} imm.</span>
                        <span>🎬 {art.videos?.length || 0} vid.</span>
                        <span>🏷️ {art.tags?.length || 0} tag</span>
                      </div>
                    </div>
                    ...
                    {art.moderatorNotes && (
                      <div className="bg-amber-50 border border-amber-200 text-amber-800 text-xs p-3 rounded-xl flex items-start gap-2">
                        ...
                        <div>
                          <span className="font-bold">Nota del Custode:</span> {art.moderatorNotes}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-slate-100">
                      <div className="flex items-center gap-2">
                        ...
                        >
                          ...
                          <span>{art.isFeatured ? 'In Evidenza' : 'Metti in Evidenza'}</span>
                        </button>
                      </div>
      ...
      {modifyingArticle && actionType && (
        <div className="fixed inset-0 z-[140] bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-[#c5a880] rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4 animate-fade-in">
            <h3 className="font-serif text-base font-bold text-[#0a1c3e]">
              {actionType === 'reject' ? 'Rifiuta Articolo' : 'Richiedi Modifiche al Cronista'}
            </h3>
            <p className="text-xs text-slate-600">
              Inserisci la motivazione o le indicazioni di revisione per l'autore <span className="font-bold">{modifyingArticle.authorName}</span>.
            </p>
