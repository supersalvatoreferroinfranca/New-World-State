    ...
    apiRouter.post('/admin/upload-branding', async (req, res) => {
      try {
        ...
        if (fileType === 'logo') {
          ...
          filenames.push('documents/branding_logo/fronte.jpg');
...
Formato: {"enQuery": "...", "itQuery": "..."}`;
        ...
        const [ytResult, wmResult, flResult, unResult, pxResult, pbResult] = await Promise.allSettled([
          ...
          (async () => {
            ...
            try {
              ...
              debugProviders.push({
                ...
                count: items.length,
                latencyMs: ytLatency,
                details: `Estratti ${items.length} filmati HD da YouTube.`
              });
              return items;
            } catch (e: any) {
              debugProviders.push({
          ...
          (async () => {
            ...
            try {
              ...
              debugProviders.push({
                ...
                count: items.length,
                latencyMs: wmLatency,
                details: `Recuperate ${items.length} immagini enciclopediche da Wikimedia Commons.`
              });
              return items;
            } catch (e: any) {
              debugProviders.push({
          ...
          (async () => {
            ...
            try {
              ...
              debugProviders.push({
                ...
                count: items.length,
                latencyMs: unLatency,
                details: `Estratte ${items.length} fotografie ad alta definizione da Unsplash.`
              });
              return items;
            } catch (e: any) {
              debugProviders.push({
          ...
          // 6. Pixabay Photo & Illustration Engine
          (async () => {
            ...
            try {
              ...
              debugProviders.push({
                ...
                count: items.length,
                latencyMs: pbLatency,
                details: `Estratte ${items.length} foto ed illustrazioni da Pixabay.`
              });
              return items;
            } catch (e: any) {
              debugProviders.push({
...
- NON restituire altri campi o modificare la struttura.`;
        ...
        execLogs.push(`[${nowStr()}] 💥 Errore irreversibile nella sessione di ricerca: ${err.message}`);
...
La cultura è un bene comune inalienabile della famiglia umana e come tale viene protetta e tramandata alle future generazioni.`,
    ...
    function getThematicImageForSlug(slug: string, title?: string): string {
    ...
    const OFFICIAL_CONSTITUTION_PDFS = [
      ...
      { lang: 'bn', name: 'সার্বভৌম বিশ্ব রাষ্ট্রের সংবিধান (বাংলা)', path: '/costitution/সার্বভৌম-विश्व-রাষ্ট্র-Bengalese.pdf', flag: '🇧🇩' },
    ...
    const CORE_SITE_PAGES = [
      {
        ...
        image: 'https://www.newworldstate.org/documents/branding_logo/fronte.jpg',
      ...
      },
      {
        ...
        changefreq: 'daily',
        priority: '0.90',
        title: 'Mappa del Sito Ufficiale (HTML Sitemap) - New World State 1.0'
      },
      {
        path: '?tab=news',
        changefreq: 'hourly',
  ...
  <channel>
    ...
    function generateComprehensiveSitemapHtml(baseUrl: string, rawArticles: any[]): string {
      ...
      const articleCards = articles.map((a, idx) => {
        const slug = a.slug || a.id;
        const cleanTitle = cleanMetaText(a.title);
        const cleanIntro = cleanMetaText(a.intro || a.content).slice(0, 140) + '...';
        const author = cleanMetaText(a.authorName) || 'Cronista Ufficiale';
      ...
      const institutionalCards = CORE_SITE_PAGES.filter(p => !p.path.includes('category=') && !p.path.includes('compliance=')).map(p => {
        ...
        return `
          <li class="sitemap-item" data-search="${escapeHtml(p.title)}">
            <a href="${escapeHtml(url)}" class="item-link">
              <span class="item-icon">🏛️</span>
              <span class="item-name">${escapeHtml(p.title)}</span>
            </a>
            <span class="item-url">${escapeHtml(url.replace(baseUrl, '')) || '/'}</span>
          </li>
        `;
      }).join('');
      ...
      const constitutionCards = OFFICIAL_CONSTITUTION_PDFS.map(pdf => {
        ...
        return `
          <li class="sitemap-item" data-search="${escapeHtml(pdf.name + ' ' + pdf.lang)}">
            <a href="${escapeHtml(fullPdfUrl)}" target="_blank" rel="noopener noreferrer" class="item-link">
...
<head>
  ...
  <style>
    :root {
      ...
      --shadow-md: 0 4px 12px rgba(10,28,62,0.07);
      --radius: 12px;
    }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .search-input:focus {
      border-color: var(--gold);
      box-shadow: 0 0 0 3px rgba(197, 168, 128, 0.35);
    }
    .search-icon {
      position: absolute;
      left: 16px;
    ...
    @media (max-width: 768px) {
      .header-banner { padding: 32px 16px 30px; }
...
<body>
    ...
    <div class="header-container">
      <div class="top-nav">
        <a href="${baseUrl}/" class="brand-group">
          <img src="https://www.newworldstate.org/documents/branding_logo/fronte.jpg" alt="Logo New World State" class="brand-logo" onerror="this.onerror=null;this.src='${baseUrl}/logo.svg';">
          <div>
  ...
  <div class="stats-grid">
    <div class="stat-box">
      ...
      <div class="stat-label">Pagine &amp; Sezioni</div>
    </div>
    <div class="stat-box">
  ...
  <main class="main-container">
    ...
    <!-- SEZIONE 1: PIATTAFORMA ISTITUZIONALE -->
    <div class="section-title-wrap">
      <h2 class="section-title">🏛️ Piattaforma &amp; Servizi Sovrani</h2>
      <span class="section-count">Principali</span>
    </div>
    <ul class="sitemap-list" id="institutionalList">
      ${institutionalCards}
    </ul>
    ...
    <!-- SEZIONE 3: CATEGORIE NOTIZIE -->
    <div class="section-title-wrap">
      <h2 class="section-title">📂 Categorie &amp; Canali Tematici</h2>
      <span class="section-count">5 Categorie</span>
    </div>
    <ul class="sitemap-list" id="categoryList">
      ${categoryCards}
    </ul>

    <!-- SEZIONE 4: COSTITUZIONE NELLE 11 LINGUE -->
    <div class="section-title-wrap">
      <h2 class="section-title">📜 Costituzione e Testi Fondamentali nelle 11 Lingue Ufficiali</h2>
      <span class="section-count">11 Documenti PDF</span>
    </div>
    <ul class="sitemap-list" id="constitutionList">
      ${constitutionCards}
    </ul>

    <!-- SEZIONE 5: CONFORMITÀ E LEGALI -->
    <div class="section-title-wrap">
      <h2 class="section-title">⚖️ Conformità Legale, Trasparenza &amp; GDPR</h2>
      <span class="section-count">5 Documenti</span>
    </div>
    <ul class="sitemap-list" id="complianceList">
      ${complianceCards}
    </ul>

    <!-- SEZIONE 6: FEEDS E CRAWLER -->
    <div class="section-title-wrap">
      <h2 class="section-title">🤖 Feed, Mappe XML &amp; Risorse per Motori di Ricerca / IA</h2>
      <span class="section-count">Crawler &amp; LLM</span>
    </div>
    <div class="feed-links-grid">
      <a href="${baseUrl}/sitemap.xml" target="_blank" class="feed-card">
        <div class="feed-name">Sitemap XML Globale</div>
        <div class="feed-desc">Indice completo in formato standard Sitemaps.org con estensioni immagini e hreflang multilingua.</div>
        <div class="feed-target">/sitemap.xml</div>
      </a>
      <a href="${baseUrl}/sitemap-news.xml" target="_blank" class="feed-card">
        <div class="feed-name">Google News Sitemap</div>
        <div class="feed-desc">Indice tempestivo ottimizzato per Googlebot-News in tutte le 11 lingue mondiali.</div>
        <div class="feed-target">/sitemap-news.xml</div>
      </a>
      <a href="${baseUrl}/sitemap.html" class="feed-card">
        <div class="feed-name">Sitemap HTML Navigabile</div>
        <div class="feed-desc">Mappa del sito interattiva accessibile a tutti gli utenti e ai crawler di ricerca.</div>
        <div class="feed-target">/sitemap.html</div>
      </a>
      <a href="${baseUrl}/rss.xml" target="_blank" class="feed-card">
        <div class="feed-name">Feed RSS 2.0 / Atom</div>
        <div class="feed-desc">Flusso di syndication per aggregatori di notizie, lettori RSS e canali informativi.</div>
        <div class="feed-target">/rss.xml</div>
      </a>
      <a href="${baseUrl}/llms.txt" target="_blank" class="feed-card">
        <div class="feed-name">File di Contesto LLM (llms.txt)</div>
        <div class="feed-desc">Documento strutturato per ingestion da parte di ChatGPT, Perplexity, Claude e Gemini.</div>
        <div class="feed-target">/llms.txt</div>
      </a>
      <a href="${baseUrl}/robots.txt" target="_blank" class="feed-card">
        <div class="feed-name">Direttive Robots (robots.txt)</div>
        <div class="feed-desc">Istruzioni di scansione e permessi per tutti i web crawler e bot di intelligenza artificiale.</div>
        <div class="feed-target">/robots.txt</div>
      </a>
    </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      const searchInput = document.getElementById('sitemapFilter');
    ...
    function findArticleBySlugOrId(slugOrId: string, articles: any[]): any | null {
      ...
      if (normTarget.length >= 3) {
        ...
        const thematicImgUrl = getThematicImageForSlug(target, readableTitle);
    ...
    // Dedicated HTML Sitemap Endpoint for human navigation, search engines & assistive devices
    ...
    // Official Constitution PDF proxy route to avoid cross-domain warnings and serve canonical PDFs
    app.get('/costitution/:filename', (req, res) => {
      const filename = req.params.filename;
      const targetUrl = `https://www.newworldstate.org/costitution/${encodeURIComponent(filename)}`;
